...
class MaintenanceResource(resources.ModelResource):
    class Meta:
        model = Maintenance
        report_skipped = True
        fields = ('id','type','date','operating_time','order_number','order_date','service_company','vehicle')

@admin.register(Maintenance)
class MaintenanceAdmin(ImportExportMixin,admin.ModelAdmin):
    resource_class = MaintenanceResource
    list_display = ('id','type','date','operating_time','order_number','order_date','service_company','vehicle')
    filter = ('date',)
...
class ComplaintsResource(resources.ModelResource):
    class Meta:
        model = Complaint
        report_skipped = True
        fields = ('id','date_failure','operating_time','node_failure','description_failure','method_recovery','repair_parts','date_recovery','downtime','vehicle','service_company')

@admin.register(Complaint)
class ComplaintsAdmin(ImportExportMixin,admin.ModelAdmin):
    resource_class = ComplaintsResource
    list_display = ('id','date_failure','operating_time','node_failure','date_recovery','downtime','vehicle','service_company')
    filter = ('date_failure',)